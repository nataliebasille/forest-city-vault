import { processVendorItems } from "@/lib/jobs/vendor-items";
...
const processVendorItemsRoute = internalProcessorRoute(() =>
  Effect.gen(function* () {
    const { requestId } = yield* RequestTrace;
    yield* processVendorItems({ requestId });
    return true;
  }),
);

export const POST = pooledRoute(processVendorItemsRoute as never);

const ACTIVE_GUARDS = new Set<string>();
const PROCESS_GUARD_KEY = "clover.process.vendor-items";
    ...
    Effect.gen(function* () {
      ...
      if (ACTIVE_GUARDS.has(PROCESS_GUARD_KEY)) {
        return yield* httpFailure(
          409,
          "Vendor item processing is already running",
        );
      }
