...
export function LoginForm() {
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    // TODO: dispatch the passwordless magic link via Supabase
    // (auth.signInWithOtp) and swap the form for a "check your inbox"
    // confirmation with a pending/disabled state. Sign-in is not wired yet — the
    // login page is design-only for now.
  }
      ...
      <div className="relative flex w-full max-w-md flex-col items-center gap-8 text-center md:max-w-sm md:items-stretch md:text-left">
        ...
        <div className="flex flex-col gap-3">
          ...
          <p className="font-body text-lg/8 text-on-light-surface-950/80 md:text-base/7 md:text-on-light-surface-50/80">
            Access is invite-only. Enter your owner email and we&apos;ll send a
        ...
        <form onSubmit={handleSubmit} className="flex w-full flex-col gap-5">
          ...
          <button
            type="submit"
            className="btn btn-solid/primary inline-flex min-h-12 items-center justify-center font-subheading text-sm font-semibold tracking-wide uppercase"
          >
