...
// `Database` is captured when this layer is built and closed over by every
// method, so the repository runs on whatever `Database` the composition root
// provides: the base pool on the pooled stack, or the saga's transaction-bound
// `Database` when the layer is rebuilt per saga inside a `withSaga` boundary.
// `Database` is therefore named honestly in the layer's requirements rather than
// hidden from the domain port.
