 ...
 * on the same {@link EventStore}/{@link EventTracker}. New aggregates are added
 ...
 */
export const RepositoriesLive = AggregateRepositoriesLive.pipe(
 ...
 * {@link Database} for the saga's scope, and each repository's backing effect
 * (`yield* Database`) captures that transaction when the layer is built at saga
 * start. Every `getById`/`save` therefore runs on the saga's transaction and
 * commits or rolls back atomically with it. A per-saga {@link EventTracker} and
 * {@link EventStore} are built alongside, so concurrent sagas never share staged
 * events.
 *
 * Requires `Saga` (provided by `withSaga`) and the base {@link Database} (from
 * `DatabaseLive`/`DatabaseTest`) to open the transaction on.
 */
export const RepositoriesSagaScoped = AggregateRepositoriesLive.pipe(
  Layer.provideMerge(EventStore.make(EventStorePersistenceLive)),
  Layer.provideMerge(EventTracker.make),
