 ...
 *
 * The {@link Database} is captured when this layer is built and closed over by
 * both methods, so appends and reads run on whatever `Database` the composition
 * root provides — the base connection normally, or the saga's transaction-bound
 * `Database` when the layer is rebuilt inside a `withSaga` boundary — and
 * therefore commit or roll back together with everything else in the saga.
